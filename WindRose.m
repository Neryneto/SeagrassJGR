...
circles     = 0:circlemax/NFrequencies:circlemax; circles = circles(2:end);% Radii of the circles that must be drawn (frequencies). We do not want to spend time drawing radius=0.
...
%% Title and Legend
title(TitleString,'color',TextColor,'fontsize',15,'fontweight',titlefontweight,'fontname',titlefontname,'color',[3 106 139]./255);         % Display a title
...
if LegendType==2                                                           % If legend type is box:
    ...
    l       = legend(axs,leyenda,'location','southoutside','orientation','horizontal','numcolumns',4);                  % Display the legend wherever (position is corrected)
...
function FrequecyLabels(circles,radius,angulo,TextColor,textfontname,escala,CenterLocation,rmin)
    ...
    for i=1:length(circles)
        text(radius(i)*c*escala+CenterLocation.X,radius(i)*s*escala+CenterLocation.Y,[num2str(circles(i))],'HorizontalAlignment',ha,...
            'fontsize',10,'verticalalignment',va,'color',TextColor,'fontname',textfontname); % display the labels for each circle
    end
%     if length(radius)>1
%         rmin = radius(1)-abs(diff(radius(1:2)));
        if rmin>0
            ...
            text(rmin*c*escala+CenterLocation.X,rmin*s*escala+CenterLocation.Y,'0','HorizontalAlignment',ha,'verticalalignment',va,...
                'fontsize',10,'color',TextColor,'fontname',textfontname); % display the labels for each circle
        end
%     end
...
function CardinalLabels(circlemax,TextColor,labels,lots_of_labels,textfontname,escala,CenterLocation)
    if ~lots_of_labels && isstruct(labels)
        text( circlemax*escala+CenterLocation.X,0+CenterLocation.Y,[' ' labels.E],'fontsize',10,'HorizontalAlignment','left'  ,...
            'verticalalignment','middle','color',TextColor,'fontname',textfontname); % East  label
        text(-circlemax*escala+CenterLocation.X,0+CenterLocation.Y,[labels.W ' '],'fontsize',10,'HorizontalAlignment','right' ,...
            'verticalalignment','middle','color',TextColor,'fontname',textfontname); % West  label
        text(0+CenterLocation.X, circlemax*escala+CenterLocation.Y,labels.N      ,'fontsize',10,'HorizontalAlignment','center',...
            'verticalalignment','bottom','color',TextColor,'fontname',textfontname); % North label
        text(0+CenterLocation.X,-circlemax*escala+CenterLocation.Y,labels.S      ,'fontsize',10,'HorizontalAlignment','center',...
            'verticalalignment','top'   ,'color',TextColor,'fontname',textfontname); % South label
