...
for iData = goodData'
    ...
    if opt.showMM > 0
        ...
        if opt.showMM == 6
            ...
            md(iData,2,1:2) = prctile(currentData,25);
            md(iData,3,1:2) = prctile(currentData,75);
    ...
    switch opt.xyOri
        case 'normal'
            if any(opt.showMM==[1,2])
                mh = plot(ah,opt.xValues+xOffset,m,'+r','Color','r','MarkerSize',12,'LineWidth',2);
            end
            if any(opt.showMM==[1,3])
                mdh = plot(ah,opt.xValues+xOffset,md,'*w','MarkerSize',12);
            end
            if opt.showMM == 4
                mh = plot(ah,opt.xValues+xOffset,m,'+k','Color','k','MarkerSize',12,'LineWidth',2);
